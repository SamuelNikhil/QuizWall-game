...
export class TeamManager {
    ...
    saveGameResult(roomId: string, questionsAnswered: number, topic: string = '', difficulty: string = 'easy'): void {
        if (this.teamDbId === null) {
            console.warn('[Team] Cannot save: no team DB id');
            return;
        }
        teamRepo.updateTeamScore(this.teamDbId, this.liveScore);
        teamRepo.saveGameSession(roomId, this.teamDbId, this.liveScore, questionsAnswered, topic, difficulty);
